...
public class ExtendedCopyOnWriteArrayList<E>
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
    private static final long serialVersionUID = -3024791131220242034L;
     ...
     */
    public ExtendedCopyOnWriteArrayList() {
     ...
     */
    public ExtendedCopyOnWriteArrayList(Collection<? extends E> c) {
        Object[] es;
        if (c.getClass() == ExtendedCopyOnWriteArrayList.class)
            es = ((ExtendedCopyOnWriteArrayList<?>)c).getArray();
        else {
            es = c.toArray();
     ...
     */
    public ExtendedCopyOnWriteArrayList(E[] toCopyIn) {
    ...
    public Object clone() {
        try {
            ...
            ExtendedCopyOnWriteArrayList<E> clone =
                    (ExtendedCopyOnWriteArrayList<E>) super.clone();
            clone.resetLock();
    ...
    public boolean addAll(Collection<? extends E> c) {
        Object[] cs = (c.getClass() == ExtendedCopyOnWriteArrayList.class) ?
                ((ExtendedCopyOnWriteArrayList<?>)c).getArray() : c.toArray();
        if (cs.length == 0)
            return false;
        synchronized (lock) {
            Object[] es = getArray();
            int len = es.length;
            Object[] newElements;
            if (len == 0 && (c.getClass() == ExtendedCopyOnWriteArrayList.class ||
    ...
    public Iterator<E> iterator() {
        return new ExtendedCopyOnWriteArrayList.COWIterator<E>(getArray(), 0);
    }
    ...
    public E dropIf(Predicate<? super E> filter, Consumer<Integer> deletedCounterConsumer) {
        ...
        synchronized (lock) {
            ...
            if (i < end) {
                ...
                final long[] deathRow = nBits(end - beg);
                int deleted = 1;
                E retVal = elementAt(es, i);
                deathRow[0] = 1L;   // set bit 0
                for (i = beg + 1; i < end; i++) {
                    E current = elementAt(es, i);
    ...
    public ListIterator<E> listIterator(int index) {
        ...
        return new ExtendedCopyOnWriteArrayList.COWIterator<E>(es, index);
    }
    ...
    public List<E> subList(int fromIndex, int toIndex) {
        synchronized (lock) {
            ...
            if (fromIndex < 0 || toIndex > len || size < 0)
                throw new IndexOutOfBoundsException();
            return new ExtendedCopyOnWriteArrayList.COWSubList(es, fromIndex, size);
        }
    }
    ...
    private class COWSubList implements List<E>, RandomAccess {
        ...
        public E set(int index, E element) {
            synchronized (lock) {
                ...
                E x = ExtendedCopyOnWriteArrayList.this.set(offset + index, element);
        ...
        public E get(int index) {
            synchronized (lock) {
                rangeCheck(index);
                checkForComodification();
                return ExtendedCopyOnWriteArrayList.this.get(offset + index);
            }
        }
        ...
        public boolean add(E element) {
            synchronized (lock) {
                checkForComodification();
                ExtendedCopyOnWriteArrayList.this.add(offset + size, element);
                expectedArray = getArray();
                size++;
            }
            return true;
        ...
        public void add(int index, E element) {
            synchronized (lock) {
                ...
                ExtendedCopyOnWriteArrayList.this.add(offset + index, element);
                expectedArray = getArray();
                size++;
            }
        }
        ...
        public boolean addAll(Collection<? extends E> c) {
            synchronized (lock) {
                ...
                boolean modified =
                        ExtendedCopyOnWriteArrayList.this.addAll(offset + size, c);
        ...
        public boolean addAll(int index, Collection<? extends E> c) {
            synchronized (lock) {
                ...
                final Object[] oldArray = getArrayChecked();
                boolean modified =
                        ExtendedCopyOnWriteArrayList.this.addAll(offset + index, c);
                size += (expectedArray = getArray()).length - oldArray.length;
                return modified;
            }
        }
        ...
        public E remove(int index) {
            synchronized (lock) {
                rangeCheck(index);
                checkForComodification();
                E result = ExtendedCopyOnWriteArrayList.this.remove(offset + index);
                expectedArray = getArray();
                size--;
                return result;
            }
        ...
        public ListIterator<E> listIterator(int index) {
            synchronized (lock) {
                checkForComodification();
                rangeCheckForAdd(index);
                return new ExtendedCopyOnWriteArrayList.COWSubListIterator<E>(
                        ExtendedCopyOnWriteArrayList.this, index, offset, size);
            }
        }
        ...
        public List<E> subList(int fromIndex, int toIndex) {
            synchronized (lock) {
                ...
                if (fromIndex < 0 || toIndex > size || fromIndex > toIndex)
                    throw new IndexOutOfBoundsException();
                return new ExtendedCopyOnWriteArrayList.COWSubList(expectedArray, fromIndex + offset, toIndex - fromIndex);
            }
        }
        ...
        private boolean bulkRemove(Predicate<? super E> filter) {
            synchronized (lock) {
                final Object[] oldArray = getArrayChecked();
                boolean modified = ExtendedCopyOnWriteArrayList.this.bulkRemove(
    ...
    private void resetLock() {
        Field lockField = java.security.AccessController.doPrivileged(
                (java.security.PrivilegedAction<Field>) () -> {
                    try {
                        Field f = ExtendedCopyOnWriteArrayList.class
